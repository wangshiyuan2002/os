...
int sys_getdents(unsigned int fd, struct linux_dirent* dirp, unsigned int count) {
	...
	int lendir = sizeof(struct dir_entry);
	int lendire = sizeof(struct linux_dirent);
	i = current->filp[fd]->f_inode;
	data = bread(i->i_dev, i->i_zone[0]);
	struct dir_entry* dir;
	for (int t = 0; t < i->i_size; t += lendir) {
		dir = (struct dir_entry*)(data->b_data + t);
		if (dir->inode) {
			dirp->d_ino = dir->inode;
			for (int x = 0; x < NAME_LEN; x++) {
				dirp->d_name[x] = dir->name[x];
			}
			dirp->d_off = 0;
